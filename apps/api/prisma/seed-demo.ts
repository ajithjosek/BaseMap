...
const COST_TYPES = ['License', 'Maintenance', 'Support', 'Infrastructure', 'Professional Services'];
...
async function main() {
    ...
    if (existing) {
      ...
      app = await prisma.application.create({
        data: {
          ...
          business_unit_id: buMap.get(appData.bu) || null,
          created_by: admin.id,
          updated_by: admin.id,
        },
      });
    }
    appMap.set(appData.name, app.id);
    ...
    if (!existingCost) {
      await prisma.applicationCost.create({
        data: {
          ...
          cost_type: COST_TYPES[Math.floor(Math.random() * COST_TYPES.length)],
          amount: appData.cost,
          currency: 'USD',
  ...
  for (const iface of INTERFACES) {
    const sourceId = appMap.get(iface.source);
    const targetId = appMap.get(iface.target);
  ...
  console.log(`Seed completed! Created ${APPS.length} applications with interfaces and capabilities.`);
}
